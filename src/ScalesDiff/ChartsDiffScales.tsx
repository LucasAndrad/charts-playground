...
export const ChartsDiffScales = memo(function ChartsDiffScales() {
  ...
  const getOptions = ({ showXaxis = false, includeMarkers = true }: OptionsProps) => {
    const data = getData(chartSize);
    const markers: any = includeMarkers ? getMockMarkers() : [];
    // const markers: any = [];
    ...
    <div className="main-container">
      <div className="container">
        ...
        {[1,2,3,4,5,6].map((item) => (
          <HighchartsReact
            key={`chart-n${item}`}
            options={getOptions({ includeMarkers: item > 2 })}
